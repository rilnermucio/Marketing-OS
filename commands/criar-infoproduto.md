...
```
- Agent(subagent_type: "mos-research", prompt: "Validação de mercado para [formato] sobre [tópico]: tamanho do nicho BR, concorrentes ativos, ticket médio praticado, dores não atendidas, fontes de tráfego dominantes. Retorne brief com 3-5 oportunidades de diferenciação.")

- Agent(subagent_type: "mos-brand", prompt: "Posicionamento e voz da marca/expert para [formato sobre tópico]. Definir arquétipo, tom, valores-chave, diferenciação competitiva. [SE marca pré-existente: usar memory em .claude/agent-memory/marketing-os-mos-brand/]")  # SÓ SE marca nova ou pivô

- Agent(subagent_type: "mos-infoproduct", prompt: "Estrutura completa do [formato]: módulos/aulas, formato de entrega, pricing strategy (por tier), bônus, garantia. Ticket alvo: [low/mid/high]. Aplicar princípios de andragogia/microlearning.")
```

## Fase 2 (sequencial, depende dos outputs da Fase 1)

```
- Agent(subagent_type: "mos-launch", prompt: "Estratégia de lançamento (PLF / semente / relâmpago / perpétuo) baseada no produto: [colar resumo mos-infoproduct], avatar, urgência: [semana/mês/trimestre]. Definir cronograma e modelo.")

- Agent(subagent_type: "mos-funnel", prompt: "Funil completo de lançamento: TOFU (CPL/anúncios) → MOFU (lead magnet/webinar/conteúdo) → BOFU (página de vendas/aplicação). Baseado em estratégia: [colar mos-launch].")
```

## Fase 3 (paralelo, depende da Fase 2)

```
- Agent(subagent_type: "mos-copy", prompt: "Página de vendas + headlines + CTAs alinhados com promessa do produto e estratégia de lançamento. Considerar memory de cliente.")
...
- Agent(subagent_type: "mos-ads", prompt: "Campanhas de tráfego pra cada fase do lançamento: pré (lista CPL) + durante (conversão) + retargeting de carrinho abandonado. Plataformas adequadas pro ticket [low/mid/high].")
```

## Fase 4: Quality Gates + Compliance + Próximos Passos

- Gates globais aplicados em todo conteúdo
- Compliance regulatório por nicho (saúde/finanças/etc.)
- Sugestão de teste A/B em headline + oferta + cronograma (mos-ab-testing)
- Setup de tracking (mos-analytics) pra métricas de lançamento
